...
def sysCall_init():
    ...
    elbow = sim.getObject("/Motor_C/Arm_C/Elbow_C")
    
    target_state = np.array([0.0, 0.0, 0.0, 0.0], dtype=np.float16)
    K = np.array([-1.1962 ,  -1.0000  , -0.9132 , -11.6092])
...
def sysCall_actuation():
    ...
    #################################
    global K, theta, theta_dot, alpha, alpha_dot
...
def sysCall_sensing():
    ...
    #################################
    global theta, theta_dot, alpha, alpha_dot
    theta = sim.getJointPosition(elbow)
    alpha = sim.getJointPosition(motor)
    theta_dot = sim.getJointVelocity(elbow)
    alpha_dot = sim.getJointVelocity(motor)
