...
function jacobian_matrices = find_jacobian_matrices(x_1, x_2, x1_dot, x2_dot)
  ...
  jacobian_matrices = {};
  ################## ADD YOUR CODE HERE ######################
  jacobian_fun_matrix = jacobian([x1_dot; x2_dot]);
  sol_col=length(solutions(1,:));
  i=1;
  while i<=length(x_1)
    jacobian_matrices(1,i)=double(subs(jacobian_fun_matrix, {x1, x2}, solutions(i,1:sol_col)));
    i=i+1;
  end
  ############################################################  
endfunction
...
function [eigen_values stability] = check_eigen_values(x_1, x_2, jacobian_matrices)
  ...
  for k = 1:length(jacobian_matrices)
    ...
    flag = 1;
    ################## ADD YOUR CODE HERE ######################
    kth_eig_values=eig(matrix);
    eigen_values(1,k)=kth_eig_values;
    for l = 1:length(kth_eig_values)
      eig_element=kth_eig_values(l);
      eig_element_conjugate=conj(eig_element);
      if eig_element+eig_element_conjugate>0
        flag=0;
      endif
     endfor
    ############################################################
